...
class KasirController {
    ...
    static async customer(req, res) {
      ...
      static async iklan(req, res, endpointku) {
        if(endpointku==='produkSaya') {
          res.render('kasir/addIklan', { endPoint: endpointku, nav: ['Iklan'], subnav: ['Iklan', 'Iklan'] })
        }
        else{
      ...
      static async createIklan(req, res,endpointku) {
        try {
            ...
            if(endpointku === 'produkSaya') {
              res.redirect('/admin/iklan/pasangIklan');
            }else{
