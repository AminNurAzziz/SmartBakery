...
io.on('connection', (socket) => {
    ...
    socket.on('updateUangPelanggan', async (data) => {
        try {
            const strukPesanan = await StrukPesanan.findOne();
            // Jika strukPesanan ada, lakukan update data sesuai dengan data yang diterima
            if (strukPesanan) {
                strukPesanan.uangPelanggan = data.uangPelanggan;
                await strukPesanan.save();
            } else {
                // Jika strukPesanan tidak ada, buat dokumen baru
                const newStrukPesanan = new StrukPesanan({ uangPelanggan: data.uangPelanggan });
                await newStrukPesanan.save();
            }
            io.to('customerRoom').emit('updateUangPelanggan', { uangPelanggan: data.uangPelanggan });
        } catch (error) {
            console.error('Terjadi kesalahan:', error);
        }
    }
    ...
    socket.on('updateKembalian', async (data) => {
        try {
            const strukPesanan = await StrukPesanan.findOne();
            // Jika strukPesanan ada, lakukan update data sesuai dengan data yang diterima
            if (strukPesanan) {
                strukPesanan.kembalian = data.kembalian;
                await strukPesanan.save();
            } else {
                // Jika strukPesanan tidak ada, buat dokumen baru
                const newStrukPesanan = new StrukPesanan({ kembalian: data.kembalian });
                await newStrukPesanan.save();
            }
            io.to('customerRoom').emit('updateKembalian', { kembalian: data.kembalian });
        } catch (error) {
            console.error('Terjadi kesalahan:', error);
        }
    });
